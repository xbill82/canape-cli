...
import { Deal, DealRelations } from "../domain/notionDeal.js";
...
export const fetchDealById = async (backend: Client, throttle: Function, id: string): Promise<Deal> => {
    ...
    const response:PageObjectResponse = await throttle(() => backend.pages.retrieve({
        page_id: id
      }))
    ...
    return await new Deal(response, relations)
}